...
from hgpsl_pooling.sparse_softmax import Sparsemax
...
class HGPSLPool(torch.nn.Module):
    ...
    def forward(self, x, edge_index, edge_attr, batch=None):
        ...
        # Graph Pooling
        original_x = x
        perm = self.topk(score, batch).node_index
        x = x[perm]
        batch = batch[perm]
        ...
        # ###added###
        #
        # x, induced_edge_index, induced_edge_attr, batch, perm, score_pooled = self.topk_pool(x, edge_index, edge_attr, batch)
        # ###added end###
