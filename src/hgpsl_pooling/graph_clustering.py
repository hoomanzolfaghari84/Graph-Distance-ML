...
def pyg_to_grakel_graph(data):
    ...
    if data.x is not None:
        ...
        for i in range(num_nodes):
            node_labels[i] = tuple(data.x[i].cpu().tolist())
    else:
        for i in range(num_nodes):
            node_labels[i] = 0
...
def cluster(dataset):
    ...
    graphs_nx = [pyg_to_grakel_graph(data,) for data in dataset]
    # graphs_gk = grakel.graph_from_networkx(graphs_nx)
    data_lables = [data.y for data in dataset]
    print("Converted {} graphs.".format(len(graphs_nx)))

    K = compute_wl_measures(graphs_nx)

    embeddings, labels_kpca = cluster_dataset_kpca_kmeans(K,n_clusters=10)

    visualize_clusters(embeddings, labels_kpca, data_lables)



def compute_wl_measures(data_list):
    # Define the WL kernel with h iterations (you can experiment with h)
    ...
    K = wl_kernel.fit_transform(data_list)
